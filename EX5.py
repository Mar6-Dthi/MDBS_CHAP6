...
class InvertedIndex:
    ...
    term_table: DefaultDict[str, Dict[int, int]] = field(default_factory=lambda: defaultdict(dict))
        ...
        with open(stoplist_path, "r", encoding="utf-8") as f:
            for line in f:
                for token in tokenize(line):
                    processed = self.preprocess_word(token)
                    if processed:
                        self.stop_words.add(processed)
    ...
    def create_index(self, directory: str, stoplist_filename: str) -> None:
        ...
        for filename in files:
            ...
            freq_in_doc: DefaultDict[str, int] = defaultdict(int)

            for raw_token in tokenize(text):
                if not raw_token or raw_token[0].lower() != "c":
                    continue

                processed = self.preprocess_word(raw_token)
                if not processed:
                    continue
                if processed in self.stop_words:
                    continue

                freq_in_doc[processed] += 1

            for term, freq in freq_in_doc.items():
                self.term_table[term][doc_id] = freq

            doc_id += 1

    def find_word(self, word: str, n: int) -> List[Tuple[int, str, int]]:
        processed = self.preprocess_word(word)
        if not processed:
            return []

        postings = self.term_table.get(processed, {})
        results: List[Tuple[int, str, int]] = []
        for doc_id, freq in postings.items():
            results.append((doc_id, self.doc_table[doc_id], freq))
    ...
    def find_wordfile(self, wordfile_path: str, n: int) -> List[Tuple[int, str, int]]:
        ...
        with open(wordfile_path, "r", encoding="utf-8") as f:
            for line_no, line in enumerate(f, start=1):
                ...
                if len(parts) != 1:
                    raise ValueError(
                        f"Dòng {line_no} trong WordFile không hợp lệ. Mỗi dòng chỉ được chứa 1 từ."
                    )

                processed = self.preprocess_word(parts[0])
                if not processed:
                    continue
        ...
        results = [(doc_id, self.doc_table[doc_id], score)
                   for doc_id, score in doc_scores.items() if score > 0]
        results.sort(key=lambda x: (-x[2], x[1]))
        return results[:n]
    ...
    def index_text(self) -> str:
        ...
        lines.append("")
        lines.append("===== TermTable =====")
        for term in sorted(self.term_table.keys()):
            postings = self.term_table[term]
            posting_str = ", ".join(
...
class App:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("EX5 - Inverted Index GUI")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Entry(setup, textvariable=self.directory_var, width=70).grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        ...
        self.index_text = self._create_text_tab(notebook, "Chỉ mục")
        ...
        self.help_text.insert("1.0",
            ...
            "2. Nhấn 'Tạo chỉ mục'\n"
            "3. Tìm 1 từ hoặc chọn file query để tìm nhiều từ\n"
            "4. File query: mỗi dòng chỉ có 1 từ\n"
        )
    ...
    def create_index(self) -> None:
        try:
            ...
            self.index_text.insert("1.0", self.index.index_text())
    ...
    def search_word(self) -> None:
        try:
            ...
            if not results:
                ...
                for rank, (doc_id, filename, freq) in enumerate(results, start=1):
                    self.result_text.insert("end", f"{rank}. doc_id={doc_id}, file={filename}, freq={freq}\n")
        ...
        try:
            ...
            if not results:
                ...
                for rank, (doc_id, filename, total_freq) in enumerate(results, start=1):
                    self.result_text.insert("end", f"{rank}. doc_id={doc_id}, file={filename}, total_freq={total_freq}\n")
